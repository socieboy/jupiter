...
<jupiter-update-role :role="role" inline-template>
    <form method="POST" action="/api/role/@{{ role.id }}" v-ajax onSuccess="updateRoleSuccess" onError="updateRoleFail">
        ...
        <div class="modal fade jupiter-update-role-modal-@{{ role.id }}" tabindex="-1" role="dialog" aria-labelledby="updateRoleLabelModal@{{ role.id }}">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title" id="updateRoleLabelModal@{{ role.id }}">
                            <i class="fa fa-pencil"></i>
                            Update @{{ role.label }}
                        </h4>
                    </div>
